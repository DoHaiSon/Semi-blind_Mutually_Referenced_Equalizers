...
for lambda = Lambda
    ...
    for exp = 1:num_exp
        ...
        [G_sb, G_sb_rc] = SB_MRE(X_SB, sig_src, Nt, Nr, N, K, M, N_p, Q, Q_rc, lambda);
    ...
    for tx = 1:Nt
        res_sb   (tx, find(Lambda == lambda)) = err_sb(tx)    / num_exp / samp_size;
        res_sb_rc(tx, find(Lambda == lambda)) = err_sb_rc(tx) / num_exp / samp_size;
    end
end
...
semilogy(Lambda, res_sb,    'LineWidth', 1);
hold on;
semilogy(Lambda, res_sb_rc, 'LineWidth', 1);
